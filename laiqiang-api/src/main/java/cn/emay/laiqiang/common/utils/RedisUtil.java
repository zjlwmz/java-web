...
//    public class SortSet {
        ...
        public long zaddlast(String key,String member){
        	 Jedis jedis = getJedis();
        	long len = jedis.zcard(key);
			double score = 0;
			if(len>0){
				Set<String> last = jedis.zrange(key, Integer.parseInt(""+len)-1, -1);
				if(last!=null && !last.isEmpty()){
					String mkey = last.iterator().next();
					score = jedis.zscore(key, mkey);
				}
			}
			long s = jedis.zadd(key,score+1, member);
			returnJedis(jedis);
			return s;
        }
...
//    public class Hash {
        ...
        public long hincrby(String key, String fieid, long value) {
            Jedis jedis = getJedis();
            long s = 0l;
            if(jedis.hexists(key, fieid)&&jedis.hget(key, fieid)!=null &&!jedis.hget(key, fieid).equals("null")){
...
//    public class Lists {
         ...
         * @return
         */
        public String sortlpop(String key){
            String value = lpop(key+"_1");
            if(value==null){
            	value = lpop(key+"_2");
            }
            return value;
        ...
        public long sortdel(String key) {
        	long count = 0;
        	Jedis jedis = getJedis();
         ...
         * @return
         */
        public long sortrpush(String key, String value,int sort) {
            Jedis jedis = getJedis();
            if(sort!=2)
            	sort = 1;
            String str = key+"_"+sort;  
            jedis.lrem(str, 0l, value);
            long count = jedis.rpush(str, value);
            returnJedis(jedis);
            return count;
        }
        /**
         ...
         * @return
         */
        public long sortlpush(String key, String value,int sort) {
            Jedis jedis = getJedis();
            if(sort!=2)
            	sort = 1;
            String str = key+"_"+sort;  
            jedis.lrem(str, 0l, value);
            long count = jedis.lpush(str, value);
            returnJedis(jedis);
            return count;
        }
        /**
        ...
        public String lindex(String key, long index) {
        	 Jedis sjedis = getJedis();
        	 String value = sjedis.lindex(key, index);
             returnJedis(sjedis);
             return value;
        }
        ...
        public long lrem(String key, int c, String value) {
        	Jedis jedis = getJedis();
            long str = jedis.lrem(key, c, value);
            returnJedis(jedis);
            return str;
        }
...
//        	 Date sdate1 = new Date();
        	...
        	/*Date sdate2 = new Date();
        	  if((sdate2.getTime()-sdate1.getTime())>1000){
			    	System.out.println("2-1==="+(sdate2.getTime()-sdate1.getTime())+"~~~~~"+key);
			    }*/
        	try {
...
//				        System.out.println(field+"-------"+data.get(field));
//				        if (!field.equals("class")&&!property.getPropertyType().isInterface()&&property.getPropertyType().toString().startsWith("class java.lang.")) {
...
//				        	Date sdate7 = new Date();
				        	String value = data.get(field);
//				        	Date sdate8 = new Date();
//				        	if((sdate8.getTime()-sdate7.getTime())>1000){
//						    	System.out.println("8-7==="+(sdate8.getTime()-sdate7.getTime())+"~~~~~"+key+"....."+field);
//						    }
				            if(value==null){
				            	continue;
				            }
    	...
    	public  List<String> zrangelist(String key, int start, int end) {
			List<String> list = new ArrayList<String>();
			Jedis jedis = getJedis();
			for(int i=0;i<end;i++){
				Set<String> keys = jedis.zrange(key, i, i);
				if(keys==null || keys.isEmpty()){
					return list;
				}else{
					list.add(keys.iterator().next());
				}
			}
			returnJedis(jedis);
			return list;
